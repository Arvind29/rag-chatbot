    ...
    <div className="flex min-h-0 flex-1">
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <div className="min-h-0 flex-1 overflow-y-auto"><div className="mx-auto max-w-4xl space-y-5 p-5 sm:p-8">
          {!messages.length && <div className="py-20 text-center"><h2 className="text-3xl font-semibold">Ask your documents</h2><p className="mx-auto mt-3 max-w-xl text-slate-500">Upload documents, index them locally, and ask grounded questions.</p></div>}
        ...
        <form onSubmit={ask} className="border-t bg-white p-4"><div className="mx-auto flex max-w-4xl gap-2"><textarea value={question} onChange={e => setQuestion(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); e.currentTarget.form?.requestSubmit(); } }} rows={1} placeholder="Ask a question..." className="min-h-12 flex-1 resize-none border px-4 py-3 outline-none" /><button disabled={loading || !question.trim()} className="bg-slate-200 px-5 font-semibold disabled:opacity-40">Send</button></div></form>
      </section>
    </div>
  </main>;
}
